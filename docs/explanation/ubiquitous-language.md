...
## Findings are a prompt, not a verdict
...
comment misleads; a misnamed class misleads *and* propagates, and every call site
...
exactly the failure mode a ubiquitous language guards against, because the agent
has no memory of the term the team agreed on last quarter. A non-zero exit code
on a naming violation gives the agent's loop the same feedback a human gets from
a red build.

The agent gets both resolutions too. Usually the fix is to use the agreed word.
Sometimes the agent is working on the thing that moved the domain, and the right
fix is a change to `dddlint.yaml`: a new canonical term, an alias retired, a
concept split across two contexts. An agent should make that edit the way a human
would, as an explicit and separate diff that says which term changed and why, so
the vocabulary change gets reviewed as a domain decision rather than slipping
through as a way to turn the build green.
